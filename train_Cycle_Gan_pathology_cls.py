...
from mymodels import Generator_unet_cls
...
from datasets import ImageDataset, ExpertDataset_label
...
parser.add_argument('--dataroot_ek', type=str, default='/home/zhangbc/Mydataspace/LST/breast/datasetX20_288/Expert_knowledge', help='root directory of the expert knowledge')
parser.add_argument('--modelroot', type=str, default='/home/zhangbc/Mydataspace/LST/breast/mymodelx288/Cycle_GAN_pathology_cls', help='root directory of the model')
...
parser.add_argument('--loadroot', type=str, default='/home/zhangbc/Mydataspace/LST/breast/mymodelx288/Cycle_GAN_pathology_cls/temp', help='continue train root directory of the model')
...
netG_B2A = Generator_unet_cls(opt.output_nc, opt.input_nc, 10, alt_leak=True, neg_slope=0.1)
...
expert_AL = Tensor(opt.batchSize2)
expert_BL = Tensor(opt.batchSize2)
...
transform_expert_ = transforms.Compose([
    transforms.RandomHorizontalFlip(),
    transforms.RandomVerticalFlip(),
    transforms.ToTensor(),
    transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])])
...
expertloader = DataLoader(ExpertDataset_label(opt.dataroot_ek, transform_expert=transform_expert_,
                                              batch_size=opt.batchSize2, unaligned=True),
...
for epoch in range(start_epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        real_expert_AL = Variable(expert_AL.copy_(expert_batch['expert_HE_label']))
        real_expert_BL = Variable(expert_BL.copy_(expert_batch['expert_Ki67_label']))
        ...
        logger.log({'loss_G': loss_G,
                    ...
                    'loss_pc': (loss_class_pathology_ABA + loss_class_pathology_BAB),
                    'loss_pf': (loss_cycle_pathology_ABA + loss_cycle_pathology_BAB),
                    'loss_embd': (loss_cycle_embd_ABA + loss_cycle_embd_BAB),
                    'loss_C': loss_C,
                    'loss_D': (loss_D_A + loss_D_B)},
                   images={'real_cycleGAN_pf_cls_A': real_A, 'real_cycleGAN_pf_cls_B': real_B,
                           'fake_cycleGAN_pf_cls_A': fake_A, 'fake_cycleGAN_pf_cls_B': fake_B})
